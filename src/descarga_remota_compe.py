...
KAGGLE_DATASET_ID = f'titanic'  #especifique el id extraido de la ruta web https://www.kaggle.com/competitions/titanic

CURRENT_DIR = Path(__file__).resolve().parent #obtener path de src

# Subimos de `src/` a `repo/` y luego añadimos `data`.
...
def download_and_extract(dataset_id: str, path: str):
    ...
    try:
        ...
        api = KaggleApi() #cree una instancia de la api
